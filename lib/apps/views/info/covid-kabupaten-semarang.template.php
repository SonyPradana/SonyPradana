...
<head>
  ...
  <script src="/lib/js/index.end.js" defer></script>
    ...
    <main>
      ...
      <article>
        <div class="header-article">
          <H1>Info Covid Kabupaten Semarang</H1>
        ...
        <div class="article body">
          <h2>Data Sebaran Di Desa
          <select
            id="input-pilih-kecamatan"
            class="textbox outline blue rounded light">
              ...
              <option value="bancak">Bancak</option>
              <option value="kaliwungu">Kaliwungu</option>
              <option value="ungaran-barat">Ungaran Barat</option>
              <option value="ungaran-timur">Ungaran Timur</option>
              <option value="bandungan">Bandungan</option>
              <option value="tengaran">Tengaran</option>
              <option value="susukan">Susukan</option>
              <option value="pabelan">Pabelan</option>
              <option value="tuntang">tuntang</option>
              <option value="banyubiru">banyubiru</option>
              <option value="jambu">jambu</option>
              <option value="sumowono">Sumowono</option>
          </select>
          </h2>
        ...
        <div class="charts">
          ...
          <div class="box-button-groub">
            <button type="button" onclick="listOfDate(90)" class="btn blue outline rounded light">3 Bulan</button>
            <button type="button" onclick="listOfDate(30)" class="btn blue outline rounded light">1 Bulan</button>
            <button type="button" onclick="listOfDate(15)" class="btn blue outline rounded light">7 hari</button>
            <button type="button" onclick="listOfDate()" class="btn blue outline rounded light">Semua</button>
          </div>
...
<script>
  ...
  const listOfDate = function(day = 0) {
    $json(`/api/ver1.0/Covid-Kab-Semarang/track_record.json?toString=true&day=${day}`)
        .then(json => {
          if (json.status == 'ok') {
  ...
  const update_chart = function(dates) {
    $json(`/api/ver1.1/Covid-Kab-Semarang/tracker-all.json?range_waktu=${dates}`)
      .then(json => {
        if (json.status == 'ok') {
          // reset
  ...
  const table = new Vue({
    ...
    methods: {
      render_table: function(kecamtan = 'ungaran-barat') {
        $json(`/api/ver1.0/Covid-Kab-Semarang/tracker-data.json?kecamatan=${kecamtan}`)
          .then(json => {
            if (json.status == 'ok') {
              this.rows      = json.data[0];
            }
          });
      }
    },
  ...
  // pilih kescamtan
  $id('input-pilih-kecamatan').addEventListener('change', (e) => {
    table.render_table(e.target.value);
  })
