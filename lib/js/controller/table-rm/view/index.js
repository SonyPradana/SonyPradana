...
function set_maks_page(maks_page){
    _maks_page = maks_page
}
...
function getData(sort, order, cure_page, search_query){
    fetchJSON(`/lib/ajax/json/private/med-rec/table-view/?sort=${sort}&order=${order}&page=${cure_page}${search_query}`)
    .then(json => {
        if( json['status'] == 'ok'){
...
function render_table(data){
    ...
    data.forEach(element => {
        ...
        th_tgl.innerText = new_date(element['tanggal_lahir'])
        ...
        let view_kk = element['nama'] == element['nama_kk'] ? `<a class="link" href="/p/med-rec/search-rm/?submit=&no-rm-kk-search=${element['nomor_rm_kk']}">view</a>` : ''
        th_action.innerHTML = `<a class="link" href="/p/med-rec/edit-rm/index.php?document_id=${element['id']}">edit</a>` + view_kk
