...
function cekRM(){
    ...
    if(nr == '') return;
    fetchJSON('/lib/ajax/json/private/med-rec/cek-nomor-rm/?nr=' + nr)
        .then( data => {
            ...
            alink.href = '/p/med-rec/search-rm/?nomor-rm-search=' + nr;
...
function cekRM_KK(){
    ...
    fetchJSON("/lib/ajax/json/private/med-rec/cari-nomor-rm-kk/?n="+ n + "&a=" + a + "&r=" + r + "&w=" + w)
        .then( data => {
            ...
            if( nrk != '' && n == nm && nrk != undefined){
                ...
                alink2.href = '/p/med-rec/search-rm/?strict-search=on&alamat-search='+ a +'&no-rt-search=' + r + '&no-rw-search=' + w + '&nama-kk-search=' + n;
...
function getDesa(desa, rw){
    fetchJSON('/lib/ajax/json/public/desa-ungaran-barat/desa.json')
    .then( data => {
        let dusun = data['data'][desa][rw]
        dusun = dusun == undefined ? '--' : dusun
        let DOM_dusun = document.querySelector('p.dusun')
        DOM_dusun.innerText = dusun
    })
}
