...
<body>
          ...
          <div class="box-left">
            ...
            <div class="container-filter">
                <form action="" method="post" class="form-filter">
                  ...
                  <div class="label label-alamat"><p>Kode Wilayah:</p></div>
                  <div class="label grub-control horizontal">
                    <label>
                      <input type="radio" id="input-dalam-wilayah" name="Filter-alamat-luar" value="0" checked>
                      Dalam
                    </label>
                    <label>
                      <input type="radio" id="input-luar-wilayah" name="Filter-alamat-luar" value="1">
                      Luar
                    </label>
                  </div>
  ...
  <script>
    ...
    $id('btnFilter').addEventListener("click", event => {
      show_panel = show_panel ? false : true
      togle_panel(show_panel)
    });

    $query('.closebtn').addEventListener("click", event => {
      show_panel = false
      togle_panel(show_panel)
    });
    ...
    function togle_panel(val) {
      if (val) {
        if (mobile_view.matches) {
          $query(".box-left").style.width = "100%"
          $query('body').style.marginLeft = "100%"
        } else {
          $query(".box-left").style.width = "250px"
          $query('body').style.marginLeft = "250px"
        }
        $query(".box-left").style.opacity = "1"

      } else {
        $query(".box-left").style.width = "0px"
        $query('body').style.marginLeft = "0px"
      }
  }
    // medai query
    ...
    $id('submit').addEventListener("click", event => {
      //get data
      ...
      let cAlamatLuar = formData.get('Filter-alamat-luar')
      ...
      && cStatusKK == null
      && dupl == null) {
          _search_query = `&all&alamat-luar=${cAlamatLuar}`
      } else {
          let query_desa = q == '' ? '' : `&desa=${q}`
          let query = `&umur=${rangeUmur}${query_desa}&status_kk=${cStatusKK}&alamat-luar=${cAlamatLuar}`
